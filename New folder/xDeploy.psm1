...
Import-LocalizedData LocalizedData -filename xDeploy.strings.psd1
...
function GetWin32Process
{
	...
	Write-Verbose "GetWin32Process - (processes) $Processes" 
    return $Processes
}
...
function GetWin32ProcessOwner
{
	...
	Write-Verbose "GetWin32ProcessOwner '(Owner.User)' $owner"
...
function GetWin32ProcessArgumentsFromCommandLine
{
	...
	Write-Verbose "GetWin32ProcessArgumentsFromCommandLine  (index)$commandLine -- (substring)$endOfCommand"
	$cmd = $commandLine += $endOfCommand | out-file c:\indexnsubstring.txt
}
...
function StartWin32Process
{
    ...
    if ($processes.Count -eq 0)
    {
        if($PSBoundParameters.ContainsKey("Credential"))
        {
            if($AsTask)
            {
                ...
                $null = Register-ScheduledTask -TaskName "xDeploy $TaskGuid" -Action $Action -User $Credential.UserName -Password $Credential.GetNetworkCredential().Password -RunLevel Highest
                $err = Start-ScheduledTask -TaskName "xDeploy $TaskGuid"
            }
            else
            {
                try
	...
	Write-Verbose "(@(GetWin32Process @getArguments)) $Processes"
}
...
function WaitForWin32ProcessStart
{
	...
	Write-Verbose "WaitForWin32ProcessStart '(value)' $value"
}
...
function WaitForWin32ProcessEnd
{
    ...
    Get-ScheduledTask | Where-Object {($_.TaskName.Substring(0,4) -eq "xDeploy") -and ($_.Actions.Execute -eq $Path) -and ($_.Actions.Arguments -eq $Arguments)} | Where-Object {$_ -ne $null} | Unregister-ScheduledTask -Confirm:$false
}
